...
extension NetworkService {
    ...
    private func fetchResponse(for request: URLRequest) async throws -> APIResult {
        ...
        do {
            ...
            let result = self.getResult(data, response)
            switch result {
            case .success(_):
                request.url?.storeCache(response: response, data: data)
            case .failure(_):
                return self.fetchCachedResponse(for: request) // show cached data
            }
            return result
        } catch {
    ...
    private func fetchCachedResponse(for request: URLRequest) -> APIResult {
        let cachedResponse = request.url?.cachedResponse
...
extension NetworkService {
    ...
    private func getResult(_ data: Data?, _ response: URLResponse?) -> APIResult {
        ...
        if statusCode == 401 || statusCode == 403 {
            return .failure(APIClientError.authError) // fallback case
        }
